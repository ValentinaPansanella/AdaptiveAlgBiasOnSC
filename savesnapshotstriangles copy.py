...
def steady_state_coevolving(model, name, max_iterations=100000, nsteady=1000, sensibility=0.00001, node_status=True, progress_bar=False):
    if not os.path.exists(f"snapshotGraphs/{name}/"):
        os.makedir(f"{name}/")
        ...
        for it in tqdm.tqdm(range(0, max_iterations), disable=not progress_bar):            
            ...
            if it == 0:
                G = model.graph.graph
                nx.write_edgelist(G, f"snapshotGraphs/{name}/edgelist {it}.csv", delimiter=",")
                with open(f"snapshotGraphs/{name}/opinions {it}.txt", "w") as opfile:
