...
def steady_state_coevolving(model, name, initial_status, nsteady=1000, max_iterations=10, sensibility = 0.00001, node_status=True, progress_bar=True):
    system_status = []
    steady_it = 0
...
for pr in [0.0, 0.5]:
    for e in [0.2]:
        for g in [0.0, 0.5]:
            ...
            status = steady_state_coevolving(model, name, initial_status, max_iterations=max_it+1, node_status=True, progress_bar=True)                
