...
def steady_state_coevolving(model, name, max_iterations=100000, nsteady=1000, sensibility=0.00001, node_status=True, progress_bar=False):
    if not os.path.exists(f"snapshotGraphs/triangles/{name}/"):
        os.mkdir(f"snapshotGraphs/triangles/{name}/")
        system_status = []
        steady_it = 0
        for it in tqdm.tqdm(range(0, max_iterations), disable=not progress_bar):            
            ...
            if it % 500 == 0:
                G = model.graph.graph
                nx.write_edgelist(G, f"snapshotGraphs/triangles/{name}/edgelist {it}.csv", delimiter=",")
                with open(f"snapshotGraphs/triangles/{name}/opinions {it}.txt", "w") as opfile:
